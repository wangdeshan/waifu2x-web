    ...
    for (const domain of["art", "art_scan", "photo"]) {
        ...
        swin[domain] = {
            ...
            scale1x: {
                ...
                scale: 1,
                offset: 8
            }, // bypass for alpha denoise
        };
        for (var i = 0; i < 4; ++i) {
            swin[domain]["noise" + i + "_scale2x"] = {
                ...base_config,
                scale: 2,
                offset: 16
            };
            swin[domain]["noise" + i + "_scale4x"] = {
                ...base_config,
                scale: 4,
                offset: 32
            };
        ...
        for (var k = 0; k < tiles.length; ++k) {
            ...
            if (single_color == null) {
                if (has_alpha) {
                    ...
                    var alpha_output = await alpha_model.run({
                        x: tile_alpha3
                    });
                    ...
                    var tile_output = await model.run({
                        x: tile_x
                    });
...
$(function () {
    ...
    async function process(file) {
        ...
        if (!onnx_runner.stop_flag) {
            ...
            output_canvas.toBlob((blob) => {
                ...
                link.href = url;
                if (document.getElementById("directdownload").checked) {
                    link.click()
                }
            }, "image/png");
        }
    ...
    $("#src").attr("status" ,"default")
    $("#src").click(() => {
        var img = $("#src").get(0);
        var status = $("#src").attr("status");
        if (status != "natural") {
            $("#src").css({
                width: img.naturalWidth,
                height: img.naturalHeight
            });
            $("#src").attr("status" ,"natural")
        } else {
            var height = 128;
            ...
            $("#src").attr("status" ,"resize")
        }
    });
    $("#dest").attr("status" ,"default")
    $("#dest").click(() => {
        var status = $("#dest").attr("status");
        if (status == "auto") {
            $("#dest").css({
                "width": "60%",
                "height": "auto"
            });
            $("#dest").attr("status" ,"resize")
        } else {
            $("#dest").css({
                "width": "auto",
                "height": "auto"
            });
            $("#dest").attr("status" ,"auto")
        }
    });
